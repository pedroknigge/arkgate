...
export const DIAGNOSTIC_DOCS_RELATIVE_PATH = 'docs/diagnostics.md' as const;

/** Schema id for serializing the catalog snapshot (agents / install projection). */
export const DIAGNOSTIC_CATALOG_SCHEMA_VERSION = '1.0' as const;
 ...
 */
export const DIAGNOSTIC_CATALOG: readonly DiagnosticCatalogEntry[] = Object.freeze([
  // ── layer / graph ────────────────────────────────────────────────────────
  entry(
  ...
  entry(
    ...
    'A string intent (or intent-like reference) names a layer that the file’s layer may not reach under the contract rules — the same plane as import edges, for event/intent coupling.',
    'Reference that intent from a layer allowed to know about it (usually an adapter or application layer), or relocate the reference — then preflight again.'
  ),
  entry(
    'LAYER_REFERENCE_VIOLATION',
    'layer',
    'Layer reference blocked (snippet / AI gate)',
    'Snippet analysis found an intent or string reference that would couple layers in a direction the architecture profile forbids.',
  ...
  entry(
    ...
    'The file’s layer lists this ambient (or its exact import dual, e.g. process / node:process) in forbiddenGlobals. Pure layers must not reach wall-clock, network, process, or similar effects directly.',
    'Inject the capability through a small port (Clock, HttpPort, Config, …), bind the implementation outside the walled layer, then preflight again.'
  ),
  entry(
    'CAPABILITY_VIOLATION',
    'capability',
    'Denied effect capability',
    'The layer denies an effect capability (network, filesystem, clock, randomness, environment, process, persistence) and the candidate uses that effect via ambient or import evidence.',
    'Define a capability port in the walled layer, bind the implementation in an adapter layer, then preflight again. Never mechanical-safe — port shape is a design decision.'
  ),
  ...
  entry(
    ...
    'publish',
    'Raw event publish',
    'Publish went through a raw string or object instead of a registered intent creator, bypassing Ark intent contracts and tooling.',
    'Publish through a registered intent creator, then run Ark again.'
  ),
  entry(
    'PUBLISH_MISSING_SOURCE',
    'publish',
    'Publish missing metadata.source',
    'A strict Ark publish call omitted metadata.source, so the publishing layer cannot be verified.',
    'Add metadata.source to the publish call, then run Ark again.'
  ),
  entry(
    'PUBLISH_SOURCE_LAYER_MISMATCH',
    'publish',
    'Publish source layer mismatch',
    'metadata.source resolves to a different layer than the file performing the publish.',
    'Use a source intent owned by the same layer as this file, or move the publish call to the owning layer.'
  ),
  entry(
    'UNKNOWN_INTENT',
    'publish',
    'Unknown intent reference',
    'Snippet analysis saw an intent string that is not registered in the intent registry / profile under check.',
    'Register the intent or use a known intent name from the project registry, then re-run the gate.'
  ),
  ...
  entry(
    ...
    'A dynamic import(expr) cannot be resolved statically and the file is not on dynamicImportAllowlist. Unresolved dynamics can hide layer edges.',
    'Rewrite to a static import when possible, or add only reviewed files to dynamicImportAllowlist after human sign-off.'
  ),
  entry(
    'DYNAMIC_REQUIRE_NOT_ALLOWLISTED',
    'safety',
    'Non-literal require',
    'A require(expr) cannot be resolved statically and is not allowlisted — same hide-the-edge risk as dynamic import.',
    'Prefer static import, or allowlist only reviewed files after sign-off.'
  ),
  entry(
    'TS_SUPPRESSION_THRESHOLD_EXCEEDED',
    'safety',
    '@ts-ignore / @ts-nocheck threshold',
    'Count of TypeScript suppressions in governed production source exceeds safety.maxTsSuppressions.',
    'Remove suppressions by fixing types, or raise the threshold only with an explicit production exception in ark.config.json.'
  ),
  entry(
    'ANY_CAST_THRESHOLD_EXCEEDED',
    'safety',
    'Explicit any cast threshold',
    'Count of explicit any casts exceeds safety.maxAnyCasts.',
    'Replace any with precise types, or raise the threshold only with a documented exception.'
  ),
  entry(
    'IN_MEMORY_STORE_IN_PRODUCTION_SOURCE',
    'safety',
    'In-memory store in production source',
    'Governed production source references an Ark InMemory* store without safety.allowInMemory — durable systems should not ship ephemeral stores by accident.',
    'Provide a durable store implementation, or set safety.allowInMemory only for an explicitly ephemeral service.'
  ),
  entry(
    'PEER_ISOLATION_DISABLED',
    'safety',
    'peerIsolation disabled on a rule',
    'A same-layer or peer rule disables peerIsolation (or omits it where required), which allows cross-slice coupling the contract otherwise blocks.',
    'Restore peerIsolation: true, or set safety.allowDisabledPeerIsolation only with a documented production exception.'
  ),
  ...
  entry(
    ...
    'Reserved / remediation-recognized code for invariant-plane failures bound to an ArkRule id (coverage path also emits INVARIANT_UNCOVERED).',
    'Fix the invariant for the ArkRule declared in arkrules/<Layer>.json, then preflight again. Do not demote without acknowledgement.'
  ),
  entry(
    'ARKRULE_SCOPE_EMPTY',
    'arkrules',
    'ArkRule appliesTo matched zero files',
    'An ArkRule’s appliesTo globs matched no governed files — the rule cannot observe what it claims to protect.',
    'Fix appliesTo globs so they match governed files, or remove the rule. Enforced empty scope fails; advisory empty scope warns.',
    { oftenAdvisory: true }
  ),
  entry(
  ...
  entry(
    ...
    'The project is adopted (required CI or explicit advisory) and has domain invariants, but ark.config.json does not name where those tests live. Green coverage without a real tests path is an empty checkbox.',
  ...
  entry(
    ...
    'preflight',
    'Unsafe change path',
    'A change set entry is not a safe, non-empty project-relative path (absolute, escape, empty, or NUL).',
    'Use canonical project-relative paths only in the atomic change set, then preflight again.'
  ),
  entry(
    'DUPLICATE_CHANGE_PATH',
    'preflight',
    'Duplicate path in change set',
    'The atomic change set lists more than one operation for the same path.',
    'Collapse to one create/update/delete per path, then preflight again.'
  ),
  entry(
    'DELETE_TARGET_MISSING',
    'preflight',
    'Delete target missing',
    'A delete operation targets a path that is not present in the supplied base tree.',
    'Remove the delete, or include the file in the base tree facts, then preflight again.'
  ),
  entry(
    'CHANGE_SET_EMPTY',
    'preflight',
    'Empty change set',
    'Atomic preflight was invoked with no create, update, or delete operations.',
    'Provide at least one change operation, then preflight again.'
  ),
  entry(
    'FACTS_IDENTITY_MISMATCH',
    'preflight',
    'Base/candidate facts identity mismatch',
    'Base and candidate resolved facts disagree on resolver, compiler, evidence requirements, or package identity — verdicts would not be comparable.',
    'Regenerate both fact snapshots with the same resolver/compiler/evidence requirements, then preflight again.'
  ),
  entry(
    'CANDIDATE_DELETE_NOT_APPLIED',
    'preflight',
    'Candidate still contains deleted path',
    'Facts claim a delete, but the candidate tree still includes the path.',
    'Ensure the candidate facts apply the delete (path absent), then preflight again.'
  ),
  entry(
    'CANDIDATE_CHANGE_MISSING',
    'preflight',
    'Declared change missing from candidate',
    'The change set declares a create/update whose path is missing from candidate facts.',
    'Include the new content in the candidate facts (or drop the operation), then preflight again.'
  ),
  entry(
    'CANDIDATE_CONTENT_HASH_MISMATCH',
    'preflight',
    'Candidate content hash mismatch',
    'The candidate file content hash does not match the hash expected for the declared change.',
    'Rebuild candidate facts from the exact proposed content, then preflight again.'
  ),
  entry(
    'UNDECLARED_CANDIDATE_CHANGE',
    'preflight',
    'Undeclared candidate change',
    'Candidate facts differ from base for a path that was not listed in the explicit change set.',
    'Declare every path that changes in the atomic change set, then preflight again.'
  ),
  entry(
    'ATOMIC_PREFLIGHT_UNAVAILABLE',
    'preflight',
    'Atomic preflight unavailable',
    'The host/MCP path could not run the atomic preflight engine (missing facts, incomplete setup, or unsupported mode).',
  ...
  entry(
    ...
    'No usable TypeScript / analysis host was available for this invocation.',
    'Install a supported TypeScript version visible to the project, then re-run. Unavailable analysis is fail-closed.'
  ),
  entry(
    'ADAPTER_NOT_ALLOWED_FOR_PORT',
    'adapter',
    'Adapter not allowed for port',
    'Runtime/port wiring selected an adapter implementation that the architecture profile does not allow for that port.',
    'Bind an allowed adapter for the port, or adjust the profile with an explicit policy decision — then re-run.'
  ),
  ...
  entry(
    ...
    'Remove or rewrite the matching code so the pattern no longer matches, then re-run the snippet gate.'
  ),
  entry(
    'FORBIDDEN_SUBSTRING',
    'snippet-policy',
    'Forbidden substring',
    'Snippet content contained a forbidden substring from the AI gate options/profile.',
    'Remove the forbidden substring, then re-run the snippet gate.'
  ),
  entry(
    'FORBIDDEN_IMPORT',
    'snippet-policy',
    'Forbidden import target',
    'Snippet imported or required a module listed as forbidden for the active profile.',
    'Import an allowed module or inject the dependency behind a port, then re-run.'
  ),
  entry(
    'POLICY_VIOLATION',
    'snippet-policy',
    'Policy engine violation',
    'A registered Policy failed on the snippet or generated code under evaluation.',
    'Adjust the code to satisfy the named policy, or change the policy only through an explicit contract decision.'
  ),
  entry(
    'EXTENSION_ERROR',
    'snippet-policy',
    'AI gate extension error',
    'A registered AICodeGate extension threw while analyzing the snippet.',
    'Fix or remove the failing extension; do not ignore extension failures as pass.'
  ),
  entry(
    'AST_ANALYZER_ERROR',
    'snippet-policy',
    'AST analyzer error',
    'Built-in AST/symbol analysis failed (host error or unexpected analyzer exception).',
    'Ensure TypeScript host and snippet are valid; re-run. If the analyzer crashes on valid input, file a bug with a minimal fixture.'
  ),
  ...
  entry(
    ...
    'config',
    'Invalid dynamicImportAllowlist',
    'dynamicImportAllowlist is present but not an array of file globs.',
    'Set dynamicImportAllowlist to an array of project-relative globs (or omit it).',
    { oftenAdvisory: true }
  ),
  entry(
    'CONFIG_INVALID_SAFETY',
    'config',
    'Invalid safety object',
    'The safety field is present but not an object.',
    'Use a safety object with optional maxTsSuppressions, maxAnyCasts, allowInMemory, allowDisabledPeerIsolation.',
    { oftenAdvisory: true }
  ),
  entry(
    ...
    'A safety threshold (maxTsSuppressions / maxAnyCasts) is not a non-negative integer.',
    'Set each threshold to a non-negative integer.',
    { oftenAdvisory: true }
  ),
  entry(
    'CONFIG_NO_LAYERS',
    ...
    'ark.config.json has no file layers, so import-boundary enforcement cannot classify files.',
    'Declare at least one layer with name + patterns (or run ark start / a preset).',
    { oftenAdvisory: true }
  ),
  entry(
    ...
    'config',
    'Layer missing name',
    'A configured layer entry has no name.',
    'Give every layer a unique non-empty name.',
    { oftenAdvisory: true }
  ),
  entry(
    'CONFIG_INVALID_FORBIDDEN_GLOBALS',
    'config',
    'Invalid forbiddenGlobals',
    'A layer’s forbiddenGlobals is not an array of strings; the entry is ignored.',
    'Use an array of strings (e.g. ["fetch", "Date.now"]).',
    { oftenAdvisory: true }
  ),
  entry(
    'CONFIG_LAYER_WITHOUT_PATTERNS',
    'config',
    'Layer without patterns',
    'A named layer has no file patterns and will never classify files.',
    'Add patterns globs that match the layer’s source tree.',
    { oftenAdvisory: true }
  ),
  entry(
    ...
    'A layer pattern is not a valid glob / failed to compile.',
    'Fix the pattern syntax for that layer.',
    { oftenAdvisory: true }
  ),
  entry(
  ...
  entry(
    ...
    'The same layer name appears more than once in configuration.',
    'Rename or merge duplicate layer entries.',
    { oftenAdvisory: true }
  ),
  entry(
    'CONFIG_RULE_UNKNOWN_FROM_LAYER',
    'config',
    'Rule unknown from layer',
    'A dependency rule references a source layer name that is not declared.',
    'Fix the rule’s from field to a declared layer name.',
    { oftenAdvisory: true }
  ),
  entry(
    'CONFIG_RULE_UNKNOWN_TO_LAYER',
    'config',
    'Rule unknown to layer',
    'A dependency rule references a target layer name that is not declared.',
    'Fix the rule’s to field to a declared layer name.',
    { oftenAdvisory: true }
  ),
  entry(
    ...
    'Some files match multiple layers at equal specificity; classification falls back to declaration order.',
    'Disambiguate overlapping patterns so each file has one clear layer owner.',
    { oftenAdvisory: true }
  ),
  entry(
  ...
  entry(
    ...
    'meta',
    'Unknown diagnostic',
    'A diagnostic lacked a stable ruleId/code; adapters may emit this fallback so agents never see an empty id.',
    'Resolve the underlying finding without weakening ark.config.json, then run Ark again. Prefer fixing the producer to emit a catalogued ruleId.'
  ),
]);

const BY_ID: ReadonlyMap<string, DiagnosticCatalogEntry> = new Map(
  DIAGNOSTIC_CATALOG.map((item) => [item.ruleId, item])
);

/** All public ruleIds in catalog order. */
export const DIAGNOSTIC_RULE_IDS: readonly string[] = Object.freeze(
  DIAGNOSTIC_CATALOG.map((item) => item.ruleId)
);

export function isKnownDiagnosticCode(ruleId: string | null | undefined): boolean {
 ...
 */
export function isCataloguedOrArkRuleFamily(ruleId: string | null | undefined): boolean {
  if (typeof ruleId !== 'string' || ruleId.length === 0) return false;
  if (BY_ID.has(ruleId)) return true;
  return ruleId.startsWith('ARKRULE_');
}

export function getDiagnosticCatalogEntry(
...
export function diagnosticDocsFragment(ruleId: string): string {
 ...
 */
export function diagnosticDocsPath(ruleId: string): string {
 ...
 */
export function catalogFixForRuleId(ruleId: string | null | undefined): string | undefined {
