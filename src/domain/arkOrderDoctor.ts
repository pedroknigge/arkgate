...
export const ARK_ORDER_DOCTOR_SCHEMA_VERSION = '1.0' as const;
...
export type ArkOrderDoctorSection = {
  schemaVersion: typeof ARK_ORDER_DOCTOR_SCHEMA_VERSION;
  notAScore: true;
  active: boolean;
  ...
  xiKeys: string[];
  residual: ArkOrderDoctorResidual;
...
export type ArkOrderStatusSlice = {
  notAScore: true;
  present: boolean;
  mode: ArkOrderDoctorMode | null;
  extraMergeTeeth: boolean;
...
export type ArkOrderDoctorConfig = {
  mode?: string;
...
function closedMode(value: unknown): ArkOrderDoctorMode | null {
  return value === 'enforced' || value === 'advisory' ? value : null;
}

function uniqueArkOrderRuleIds(
  findings: readonly { ruleId?: unknown }[] | null | undefined
): string[] {
  const seen = new Set<string>();
  if (!Array.isArray(findings)) return [];
  for (const finding of findings) {
    const id = finding?.ruleId;
    if (typeof id !== 'string' || !isArkOrderRuleId(id) || seen.has(id)) continue;
    seen.add(id);
  }
  return [...seen].sort((left, right) => (left < right ? -1 : left > right ? 1 : 0));
}

function extraFromConfig(arkOrder?: ArkOrderDoctorConfig | null): {
  present: boolean;
  mode: ArkOrderDoctorMode | null;
  roots: number;
  layers: number;
  ...
  const xiKeys = Array.isArray(arkOrder.xiKeys)
    ? arkOrder.xiKeys.filter((key): key is string => typeof key === 'string' && key.length > 0)
    : [];
  return {
    present: true,
 ...
 */
export function summarizeArkOrderSection(
  input: {
    arkOrder?: ArkOrderDoctorConfig | null;
  ...
  const extraMergeTeeth =
    extra.present && extra.mode === 'enforced' && extraMergeTeethAllowed(input.classification);
...
export function formatArkOrderDoctorLines(section: ArkOrderDoctorSection): string[] {
  ...
  if (section.residual.count > 0) {
    ...
    lines.push(
      'Residual: none on this scan (not a score — green extras ≠ a frozen billing plan).'
    );
  }
