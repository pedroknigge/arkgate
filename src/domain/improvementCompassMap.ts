/**
 * Improvement compass — fact → lens projection (Domain pure).
 *
 * Split from improvementCompass.ts (DF03): smell/violation/count mappers stay
 * together so residual mapping is one place to test and regenerate for CLI.
 *
 * Pure Domain: no fs, fetch, Date.now, process — inject facts as input.
 */

import {
  IMPROVEMENT_COMPASS_LENS_LABELS,
  IMPROVEMENT_COMPASS_OUT_OF_SCOPE_LENSES,
  IMPROVEMENT_COMPASS_OUT_OF_SCOPE_SET,
  IMPROVEMENT_COMPASS_OUT_OF_SCOPE_SUMMARIES,
  IMPROVEMENT_COMPASS_RESIDUAL_SORT_PRIORITY,
  IMPROVEMENT_COMPASS_TOP_RESIDUAL_CAP,
  IMPROVEMENT_LENS_IDS,
  type ImprovementCompassEvidence,
  type ImprovementCompassFacts,
  type ImprovementCompassMutableLens,
...
function pushEvidence(
  lens: ImprovementCompassMutableLens,
  source: string,
  ref: string,
  detail?: string
): void {
...
function markResidual(
  ...
  if (IMPROVEMENT_COMPASS_OUT_OF_SCOPE_SET.has(lens.id)) return;
...
export function createInitialImprovementCompassLenses(): ImprovementCompassMutableLens[] {
...
function mapDesignSmells(
  byId: Map<ImprovementLensId, ImprovementCompassMutableLens>,
  ...
  for (const smell of smells) {
    ...
    const attach = (
      ...
      if (!lens || IMPROVEMENT_COMPASS_OUT_OF_SCOPE_SET.has(lensId)) return;
...
function isTypeOnlyPlacementDebt(v: ImprovementCompassViolationFact): boolean {
  // Product voice: type-only edges are placement debt (failsStrict:false), not runtime coupling.
  if (v.failsStrict === false) return true;
  if (v.typeOnly === true) return true;
  return false;
}
...
function mapViolations(
  ...
  for (const v of violations) {
    ...
    if (isTypeOnlyPlacementDebt(v)) {
      attach(
        'modularity',
        'Type-only placement debt remains — prefer SharedTypes / owning layer (not runtime coupling).',
        {
          kind: 'skill',
          ref: '/ark-place',
          summary:
            'Place shared types in a layer both sides may import; type-only debt is not a value edge.',
        }
      );
      continue;
    }
    ...
    if (upper.startsWith('ARKRULE_') || upper === 'INVARIANT_UNCOVERED') {
      attach(
        ...
        'ArkRules structure / invariant residual inside a layer.',
        {
          ...
          ref: '/ark-autopilot',
          summary:
            'Label [ArkRules]; structure fixes are judgment — never invent mechanical-safe.',
        }
      );
      attach(
        'domain',
...
function mapCountsAndFlags(
  ...
  if (
    ...
    markResidual(
      m,
      'Substantial frozen residual remains under the baseline — review debt honestly.',
      {
        kind: 'command',
        ref: 'ark-check --doctor',
  ...
  if (facts.designWeak === true && facts.goldenPatternPresent === false) {
    ...
    markResidual(
      mod,
      'Design-weak without a golden pattern — new code lacks a placement norm for the AI.',
      {
        kind: 'skill',
        ref: '/ark-place',
 ...
 */
export function projectImprovementCompassFacts(
  byId: Map<ImprovementLensId, ImprovementCompassMutableLens>,
  facts: ImprovementCompassFacts
): void {
...
export function lockImprovementCompassOutOfScope(
  byId: Map<ImprovementLensId, ImprovementCompassMutableLens>
): void {
...
export function sortImprovementCompassEvidence(
  lenses: readonly ImprovementCompassMutableLens[]
): void {
...
export function finalizeImprovementCompassTopResidual(
  lenses: readonly ImprovementCompassMutableLens[]
): ImprovementLensId[] {
