/**
 * Pure co-pilot remediation classifier + fix-class enrichment.
 *
 * **Canonical algorithm** for CLI (`bin/lib/remediation.mjs` generated) and unit tests.
 * Zero Node I/O — pure functions of one violation object.
 *
...
export type RemediationKind =
  ...
  /**
   * RN05: add an existing call-site literal to uses/reactsTo/raises/sends.
   * Mechanical-safe only when `target` is that non-empty literal. Inventing a
   * new emit/handle/depend stays judgment.
   */
...
function arkRunCallSiteName(violation: ArkViolationLike): string | undefined {
  return typeof violation.target === 'string' && violation.target.trim().length > 0
    ? violation.target.trim()
    : undefined;
}

function isArkRunDeclarationListSafe(violation: ArkViolationLike): boolean {
  return (
...
function arkRunNextAction(violation: ArkViolationLike): string {
  const target = arkRunCallSiteName(violation);
  const fromLayer =
    typeof violation.fromLayer === 'string' && violation.fromLayer.length > 0
      ? violation.fromLayer
      : undefined;
...
function arkRunEnthusiastHint(violation: ArkViolationLike): string {
  const target = arkRunCallSiteName(violation);
  switch (violation.ruleId) {
    case 'ARKRUN_MISSING_ROOT':
      return target
...
export function deterministicNextAction(violation: ArkViolationLike): string {
  switch (violation.ruleId) {
    ...
    case 'INVARIANT_UNCOVERED':
      return `Fix the structure or invariant for ${
        typeof violation.arkruleId === 'string' && violation.arkruleId.length > 0
          ? violation.arkruleId
          : 'the ArkRule'
      } (declared in ${
        typeof violation.arkruleSource === 'string' && violation.arkruleSource.length > 0
          ? violation.arkruleSource
          : 'arkrules/<Layer>.json'
      }), then preflight again. Do not demote the rule without a hash-bound policy acknowledgement.`;
    case 'ARKRUN_MISSING_ROOT':
    case 'ARKRUN_KERNEL_IN_DOMAIN':
    case 'ARKRUN_DIRECT_NEW':
    case 'ARKRUN_UNDECLARED_EMIT':
    ...
    default:
      if (typeof violation.ruleId === 'string' && violation.ruleId.startsWith('ARKRULE_')) {
        return `Fix the ArkRule ${
          typeof violation.arkruleId === 'string' ? violation.arkruleId : violation.ruleId
...
export function classifyRemediation(violation: ArkViolationLike | null | undefined): RemediationVerdict {
  ...
  if (ruleId === 'LAYER_IMPORT_VIOLATION') {
    ...
    if (edgeKind === 'require' || edgeKind === 'dynamic-import') {
      return {
        ...
        confidence: 0.75,
        rationale:
          'Runtime module load (require/import()) still executes the target file — not auto-safe; rewrite to a static import type if appropriate.',
      };
    }
  ...
  if (
    ...
    return {
      ...
      confidence: 0.85,
      rationale:
        'ArkRule structure/invariant findings are never mechanical-safe — restore private state, factory shape, event publish, coverage, or redesign the aggregate with judgment.',
    };
  }
  if (violation && isArkRunDeclarationListSafe(violation)) {
    return {
      ...
      rationale:
        'Call-site literal already exists; adding it to the declaration list is behavior-preserving. Inventing a new emit/handle/depend stays judgment.',
    };
  }
  if (
    ...
    return {
      ...
      confidence: 0.85,
      rationale:
        'ArkRun usage, construction, and homemade-transport findings are never mechanical-safe. Declaration-list edits are mechanical-safe only when the call-site literal is already present as target.',
    };
  }
...
export function enrichViolationWithFixClass<T extends ArkViolationLike>(
  ...
  switch (violation.ruleId) {
    ...
    case 'INVARIANT_UNCOVERED':
      enriched.fixClass = 'review-contract';
      enriched.effort = 'medium';
      enriched.enthusiastHint = `ArkRule ${
        typeof violation.arkruleId === 'string' ? violation.arkruleId : 'rule'
      } failed${
        typeof violation.arkruleSource === 'string' ? ` (from ${violation.arkruleSource})` : ''
      }. Restore the declared structure or add a covering test/symbol — do not demote without acknowledgement.`;
      break;
    case 'CIRCULAR_DEPENDENCY':
