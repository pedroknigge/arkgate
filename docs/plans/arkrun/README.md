...
`RN17` **done** in **4.7.4** — `@arkgate/runtime` on npm `experimental`. Does not close `Z09` /
...
| Declare every raise / handle / depend / send | Companion `register()` takes `uses` / `reactsTo` / `raises` / `sends`; gate sensors already consume file-scoped lists | Graph slices consume those declarations |
...
| Partial diagrams | `requestGraph({ slice, nodeIds, degreesOfSeparation, include, exclude })` + Mermaid helper | Inspector `GET /graph` |
| Dev inspector | Opt-in `startInspector()`: `127.0.0.1`, production veto, SSE + snapshots + `/graph`, lazy HTTP | Graph slices (`RN13`) |
...
| `RN12` | M | RN10 | **done** — Inspector: localhost, production veto, SSE + snapshots; lazy load |
| `RN13` | M | RN10 | **done** — `requestGraph` slices (process/technical, degrees, query) + Mermaid helper |
...
| `RN17` | M | RN16 | **done** — First experimental npm publish of `@arkgate/runtime` (`experimental` dist-tag, never `latest`); `/ark-runtime` can install from the registry |
