...
**arkgate@3.8.2** (current stable) installs a physically distinct exact TypeScript 6 analysis host
...
[3.8.2 release notes](releases/3.8.2.md).
...
`3.8.2` is the current stable version when this guide was updated; prefer `arkgate@latest` for a
...
| TS7 plan/check says `partial` or `unavailable` | Do not accept the plan as green; upgrade to **arkgate@3.8.2** or later, then require `completeness: complete` |
| pnpm blocks new package age | Wait for cooling-off or prefer `arkgate@latest`; if policy requires an exact pin, check `npm view arkgate version` and pin that version (currently `arkgate@3.8.2`) |
