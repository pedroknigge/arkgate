...
| **Enforcement state (Z06)** | `doctor.writePath.enforcementState`; `arkgate/schema/enforcement-state`; public `ArkEnforcementState` type | Schema `1.0` separates analyzed/configured/installed from runtime/provider facts. Workflow names never imply required branch status; absent provider evidence remains `unverified`. The T05 ladder is retained as a derived compatibility view. |
