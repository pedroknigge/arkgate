...
| **Contract health (W01)** | `ark-check --doctor --json` → `doctor.contractHealth`; optional `.ark/contract-smell-acks.json` | Additive, **advisory only** — meta-lint of the contract itself; never changes the verdict, `designFitness`, or `patternBets`. Stable smell ids: `contract-bidirectional-allow`, `contract-peripheral-depends-core`, `contract-lateral-adapter-allow`, `contract-dead-rule`; each has `evidence[]`, `fix`, `message`, and plain-language `outcome`. Acknowledgments live in the sidecar (`{ acks: [{ id, edge, reason }] }`, bidirectional edges order-insensitive); **absent is normal**; malformed → `ackFile.invalid: true`, acks ignored, never silent suppression. |
