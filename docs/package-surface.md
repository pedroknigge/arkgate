...
| **`arkgate/schema/analysis-result`** | Public CLI/MCP/hook diagnostic envelope (`schemaVersion`, `valid`, `diagnostics`) | Schema `1.1` adds optional deterministic `nextAction`. The immutable `1.0` fixture protects the original rule, severity, location, and evidence shape; consumer-owned TypeScript diagnostics without `nextAction` remain valid. |
