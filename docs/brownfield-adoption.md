...
Adopting Structrail on a large, pre-existing codebase is a different job from a greenfield one: the
...
order. This playbook is the sequence that keeps you honest (and is what `/structrail-adopt` runs).

The rule underneath all of it: **`structrail.config.json` is authoritative.** A green check that
...
```bash
structrail-check --init          # detects layer directories; proposes a layer for each ungoverned one
```
...
```bash
structrail-check --root . --config structrail.config.json --json   # read `summary`
structrail-check --doctor                                    # or the consolidated health view
```
...
## 3. Make the contract real (via `/structrail-contract`)

- **Classify the ungoverned tree.** `structrail-check --coverage` leads with `Governed: N%` and
...
```bash
structrail-check --update-baseline    # writes .ark-baseline.json — commit it
```

If Structrail still refuses (a single edge dominates), the contract is still wrong — go back to
step 3; don't `--force` past it. From now `structrail-check --baseline` fails only on NEW
violations — the ratchet only moves toward zero.

## 5. Burn down, in order

`summary.edges` is the burn-down order. Prefer `structrail-check --plan`: it tags each step
...
`/structrail-fix` resolves each cluster at the root cause; fixing a frozen violation shrinks the
baseline permanently. Re-freeze lower with `--update-baseline` as you go.

## What Structrail does NOT do here

Structrail reorganizes and governs code — it never touches your data model. Migrating raw SQL to a
repository moves the same query to another file; the schema, migrations, and the database are
untouched. And the burn-down itself is the team's work (or a codemod, or an agent loop) — Structrail
