...
**Date:** 2026-07-22<br>
**Scope:** project<br>
**Intent:** audit → selective documentation update (queue hygiene after 3.8.0)<br>
...
1. No bounded current-source claim remains contradicted. Field-proof residual is still partial
   under C-028: Z07/Z08/Z10 claim evidence is earned in published **3.8.0**; Z09 remains a
   **parked claim gate** for retained adoption and independently signed review identity only
   (residual `RB-11`). It is not an engineering `doing` slot until a signed matrix and ≥8
   consented adopters are preregistered.
2. `@arkgate/runtime` is not currently present in the npm registry. Its first publication remains
   ...
   parked candidate `K01` retains three confirmed intra-process commit gaps. Runtime is not the
   product wedge; deprecated root forwarders stay until major 4.0.
3. Self-hosted design residual is judgment-only hygiene, not a ROADMAP `todo` or release
   blocker. The `src/domain/analysis.ts` god-module plan-B pilot cluster was applied (IR + facts
   split); residual god-module evidence is the package barrel `src/index.ts` only. Parked
   Y06/Y09 stay behind field gates; Y07 is low priority; Y10 is archived until field demand + ADR.

**Recommended next Intent:** no product-implementation `doing`. Keep residual `RB-11`/`Z09` as a
claim gate only; promote Z09 only when its preregistration gate is met. Do not start parked
Y06–Y10 or K01 as collateral. Ordinary corrective patches may ship without waiting for Z09.
...
| C-017 | Project TS5/6/7 compilers stay project-owned while a packed TS7 install retains a usable JS-API analysis host and fails closed when analysis is incomplete | [TypeScript support](../typescript-support.md) | exact `typescript-ark-host` alias · `bin/lib/typescript-host.mjs` · schema 1.2 completeness · packed Node/package-manager/TS matrix | OK | All 36 packed cells passed on source `228dd893` in CI run `29655190747`; published in **3.8.0** |
| C-018 | All retained plan seeds are indexed with current shipped status | [Hub plans](../../AGENTS.md#product-plans-library-epic-queue-seeds) | `docs/plans/` · ROADMAP completed phases | OK | keep roadmap authoritative; Z09 is parked claim gate after 3.8.0 hygiene |
...
| C-020 | Current release identity is aligned at 3.8.2 | [Release notes](../releases/3.8.2.md) | package/lock/version/server metadata | OK | release gate owns future sync |
...
| C-026 | Every gallery starter can be copied, installed, and checked using its documented commands | [Examples](../../examples/README.md) | one frozen catalog · checksum-verified npm/pnpm/Yarn clean-room reports | OK | 18/18 cells and 198/198 stages passed on source `3423758` in CI run `29667803023`; published in **3.8.0** |
...
| C-028 | V03/V05/B01 evidence measures time to the real merge gate, observed false blocks/bypasses, and independent review | [Roadmap evidence](../../ROADMAP.md#success-metrics) | Z08 immutable 144-cell causal ledger/report · measured classifications · historical beta declaration check | Partial | Z07/Z08/Z10 claim evidence earned in 3.8.0; residual `Z09`/`RB-11` still owns retained adoption and verified reviewer identity only |
| C-029 | New/worsened supported design smells can be ratcheted without blocking historical residual, and host hardness requires runtime/provider proof | [Package surface](../package-surface.md) · [AI gates](../ai-gates.md) | design-delta schema/types · semantic UI-rule detector · hook/MCP/CLI parity corpus · enforcement-state 1.1 | OK | Closed with Z10 / RB-12 on PR #89; published in 3.8.0 |
...
is OK. Published **3.8.0** includes that distribution correction (see [3.8.0 notes](../releases/3.8.0.md)).
...
`NoCoverage` mutants in the critical ranges.

Current source and published **3.8.0** implement Z10's new-code design enforcement and
runtime-proven host hardness. [ROADMAP Phase Z](../../ROADMAP.md#phase-z--enforcement-truth-at-speed)
closes Z10 and `RB-12` on exact-head PR #89 CI/Security evidence. Residual field-proof under C-028
is only longitudinal retention and verified independent identity: Z09 is **parked** (claim gate)
and residual `RB-11` is scoped to those two outcomes. Prose is not accepted as implementation
evidence.
...
|-----------------|-------|------------|
| Contradicted | A packed TS7 consumer could lose the intended JS-API fallback and report an unavailable plan as met | Current source and published **3.8.0** use a distinct exact TS6 host and required fail-closed completeness; all 36 packed CI cells passed on source `228dd893`. Historical 3.7.0 tarballs still lack the correction |
...
- npm-tarball Markdown links: **PASS** — all relative targets from packed Markdown files are
  present in the `arkgate@3.8.0` dry-run inventory (historical verification also held for 3.7.0).
...
- Z10 current-source candidate: **PASS exact head** — PR #89 head `357e282`, CI run
  ...
  review pass. Z10 and `RB-12` are closed; published in **3.8.0**.
...
- Registry check: **OBSERVED** — `arkgate@latest` is `3.8.0`; querying
