# Structrail — Agent Integration Guide

**Structrail** (`structrail`) — architecture co-pilot for AI TypeScript. This guide describes how AI
agents and codegen tools safely interact with write hooks, advisory MCP tools, CI, and `/structrail-*`
skills. Guarantees differ by host; start with the
[canonical host support matrix](../README.md#host-enforcement-support).

CLI names: **`structrail` / `structrail-check` / `structrail-mcp`**. TypeScript
**5.x / 6.x / 7.x** as the project compiler:
see [typescript-support.md](typescript-support.md).

## Architecture playbook and `structrail-check --recommend`

Before generating project structure, agents should read the **tool-agnostic application
shape** that fits the repository — not a vendor stack label. Structrail ships a versioned playbook
...
- a named Structrail preset (`hexagonal`, `layered`, `feature-sliced`, `monorepo`, `ui-surface`, `vertical-slice`, or `ddd-bounded-contexts`),
...
`--recommend` does not require `structrail.config.json`. It exits `0` and prints a progressive
adoption plan: archetype id, preset, `confidence`, `runnerUp`, `why` (shape signals),
`adoptInOrder.phase1`, `firstCommand` (`structrail init --archetype …`), and `checkCommand`.

Human output highlights phase-1 layers and the analogy; JSON is the stable contract for
MCP `structrail_recommend` and the `/structrail-architect` skill.

### Terminal onboarding (Phase B)

```bash
npx structrail start --yes                             # preferred: shape → contract → **day-zero origin** → gates → plan
npx structrail init --archetype crud-product --yes   # non-interactive: shape → preset → origin → gates
npx structrail init                                    # TTY wizard: pick application shape (1–8), not a framework
npx structrail-check --doctor                          # includes "New here?" when coverage is low or config is fresh
npx structrail-check --report beginner.html --beginner # simplified HTML for enthusiasts
npx structrail-check --watch                           # debounced re-check when governed files change
```

**Day-zero origin (2.12+):** as soon as `structrail.config.json` exists, `structrail start` / `structrail init`
...
**Default agent co-pilot:** `/structrail-autopilot` (explore-first, dual plan A remediation + B
pattern bets). Recon without applying: `/structrail-explore`. Never treat empty `--plan` as
“architecture healthy” without the explore pass.

`structrail init --archetype <id>` maps playbook ids to named presets (`hexagonal`, `layered`,
`feature-sliced`, `monorepo`). With `--yes` and no archetype, Structrail auto-selects from
`--recommend` scoring.

`structrail-check --json` violations include enthusiast-oriented fields when present:
...
Structrail does **not** reimplement general ESLint rules. `--doctor` / adoption gaps **do**
...
order (universal): `lint` → `typecheck` → `structrail-check` / `check:architecture` → `build`.
...
```bash
npx structrail-check --suggest-include --json    # workspaces + nested package.json+TS roots
npx structrail-check --adopt-contract --write  # expand include + UI patterns (no rule weakening)
npx structrail-check --coverage
```

Polyglot repos: Structrail only governs TypeScript/JS. Point include at package roots that have sources.
...
### MCP `structrail_recommend` and `/structrail-architect`

The `structrail-mcp` server exposes **`structrail_recommend`** — same JSON as
`structrail-check --recommend --json`. Call it (or invoke `/structrail-architect`) before
generating project structure on greenfield or early-adoption repos.

`structrail-mcp --session-context` appends a one-line enthusiast hint when governed
...
The `/structrail-architect` skill ships in `templates/skills/structrail-architect.md` and installs
via `structrail-check --install-agent-gates`.

### Adoption plan artifact

```bash
npx structrail-check --recommend --write-plan
# writes structrail-adoption-plan.json (optional commit; never weakens the gate)
```

Includes `archetype`, `preset`, `adoptInOrder`, `galleryStarter`, and suggested
`policyPack` (`enthusiast-<preset>`).

### Enthusiast policy packs

```bash
npx structrail-check --list-policy-packs
npx structrail-check --apply-policy-pack enthusiast-hexagonal   # or layered, feature-sliced, monorepo
```

Packs delegate to the same preset factories as `structrail init --preset`; layer
...
1. Run `structrail-check --recommend --json` or MCP `structrail_recommend`.
2. Read `archetype`, `preset`, and `adoptInOrder.phase1` — scaffold only those directories first.
3. Run `structrail init --archetype <id> --yes`, `--apply-policy-pack enthusiast-<preset>`, or `structrail init --preset <preset> --yes` when no `structrail.config.json` exists.
4. Optional: `--write-plan` for `structrail-adoption-plan.json`; copy a gallery starter from `examples/README.md`.
5. Use `/structrail-place` or `structrail_place` for individual files after the contract exists.
6. Verify with `structrail-check --root . --config structrail.config.json --strict`.
...
| Surface | Role |
|---------|------|
| MCP **`structrail_prepare_write`** | Place + constrain + validate + optional `autoPatch` + `judgmentBrief` + contentHash in one call |
| Write-gate **`autoPatch`** | Mechanical-safe **import type** rewrites only; post-patch revalidation green or discarded |
| PreToolUse **`--hook-repair`** | On deny: `STRUCTRAIL_REPAIR_JSON` / `STRUCTRAIL_AUTOPATCH_JSON` on stderr (still exit 2 — never silent write) |
...
directories (`utils/`, `lib/`) must be classified explicitly via `/structrail-contract`.

**Brownfield** (existing messy repo): use `/structrail-adopt` and [brownfield-adoption.md](brownfield-adoption.md), not `/structrail-architect`.

## Supported agent hosts

Wire write-gate + MCP + `/structrail-*` skills with:
...
| OpenAI Codex | `$CODEX_HOME/config.toml` (global; absolute `--root`; multi-project → secondary `structrail_<slug>` unless `--force`; doctor defers non-temp home gaps when session host ≠ Codex — see [ai-gates.md](ai-gates.md)) | `$CODEX_HOME/prompts` (`--codex-home`; fix when using Codex) |
| **Grok Build** | `.grok/hooks/structrail-write-gate.json` + `.grok/config.toml` / `.mcp.json` | `.grok/skills/<name>/SKILL.md` |

This is a path reference, not a guarantee table. Full copy-paste setups:
[ai-gates.md](ai-gates.md). Skill inventory: main [README](../README.md#other-skills-only-when-you-need-them).
...
presented as production-ready. If you are evaluating it, prefer `createStrictStructrailKernel()`. It
...
const structrail = createStrictStructrailKernel();
// ... define intents, event contracts, metadata, projections, and workflows through structrail.*

const contract = structrail.manifest().toJSON();
// contract.intents, policies, entities, graph, architecture, eventContracts,
// contract.observability, projections
```

Use `structrail/runtime` when evaluating the experimental kernel (root `structrail` still re-exports
for compatibility). Package surface policy: [package-surface.md](package-surface.md).

Agents should read `contract` and `structrail.observability.report()` before generating or modifying code.
...
| `Kernel.*` | Structrail-owned governance signals | `Kernel.PolicyViolation` |
...
structrail.eventContracts.register({
...
const publisher = structrail.publisher('Application.PlaceOrder');
...
Agents should prefer `structrail.publisher(sourceIntent).publish(...)` over direct
...
```ts
structrail.eventBus.registerInterceptor(OrderPlaced, ({ intercept }) => {
  intercept({ auditTag: 'checkout' });
}, 'audit-tag');
```

If an interceptor overwrites an existing field or violates the registered event
contract, Structrail records `interceptor.error` and keeps delivering the original event.
...
`structrail-mcp` enables these checks automatically when TypeScript is available. The exact supported
...
Use `structrail-check` in CI for repository-level checks that need real file paths:

```bash
npx structrail-check --root . --config structrail.config.json
```

### Monorepo tooling (Turborepo / Nx)

Use **`structrail init --preset monorepo`** (or archetype `multi-app-workspace`). Structrail does **not**
reimplement the Nx project graph. It maps conventional trees:

| Tooling | Typical roots | Structrail `include` fallback |
|---------|---------------|------------------------|
...
`structrail init --preset clean-architecture` and `--preset onion-architecture` are **aliases** of
...
- Enforced by `structrail-check`, `structrail/eslint`, and `structrail-mcp` when paths resolve.
...
```bash
npx structrail-check --print-config eleven-layer
```
...
`structrail-check` resolves imports through the TypeScript module resolver against your
...
`structrail-check --json` also reports `warnings` for incomplete governance coverage: missing
...
Rules: `structrail/no-domain-infra-imports` (layer edges from `structrail.config.json`, same semantics as
`structrail-check`), `structrail/no-forbidden-globals` (per-layer `forbiddenGlobals`),
`structrail/no-raw-event-publish`, and `structrail/require-publish-source`. See [ai-gates.md](ai-gates.md).
...
`structrail.observability.report()` compares declared productions with observed runtime
flows. Use `observedButUndeclared` as a high-signal review queue for hidden coupling.

For tests, use `createStructrailTestHarness(structrail)` to inspect events, traces, audit,
outbox, and observability snapshots without reaching into private internals.

## Extension Points (External Layers)

Implement these interfaces in **external** packages — not inside the Structrail core:
...
**boundary-only on purpose**: Structrail governs what they may import and publish, but does
...
`structrail-mcp` exposes Structrail over MCP (JSON-RPC over stdio; gate host needs a
JS-API TypeScript — nested or project) so a host can gate
the write path:

```bash
npx structrail-mcp --root . --config structrail.config.json [--manifest structrail.manifest.json]
```

- **Resource `structrail://manifest`** — contract discovery. Serve your exported
  `structrail.manifest().toJSON()` via `--manifest`, or omit it to get the 11-layer profile
  (layers + rules) as the default contract.
- **Tool `structrail_recommend`** — no args. Returns the deterministic application-shape plan
  (archetype, preset, phased adoption, analogy). Same as `structrail-check --recommend --json`.
...
For hook-based enforcement, `structrail-mcp --hook` runs one-shot: it reads a PreToolUse payload
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx structrail-mcp --hook --root \"$CLAUDE_PROJECT_DIR\""
          }
        ]
      }
    ]
...
Register the server itself in `.mcp.json` so the agent can read `structrail://manifest` and call
`validate_code` on demand:

```json
{
  "mcpServers": {
    "structrail": { "command": "npx", "args": ["structrail-mcp", "--root", ".", "--config", "structrail.config.json"] }
  }
}
```
...
1. **Read** manifest via `structrail.manifest().toJSON()`
...
4. **Validate repository** with `structrail-check --root . --config structrail.config.json`
5. **Lint** with `structrail/eslint` recommended rules
6. **Wire** relationships via `registry.define(..., { dependsOn, produces })`
7. **Register** event contracts before publishing in strict mode
8. **Observe** runtime via `bus.getTrace()`, `auditTrail.query()`, outbox records, projection checkpoints, and `structrail.observability.report()`
