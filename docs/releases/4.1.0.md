...
No config migration. Optional after upgrade: re-run `ark start` / `--init` on Next trees so API
shell patterns land if you regenerate from presets; existing hand-tuned configs keep working
...
reclassified as placement debt on the violations list (`failsStrict: false`, diagnostic severity
warning — not merge-blocking). peerIsolation type-only edges remain hard. Intentional product
policy — not a silent weaken of runtime coupling checks.
