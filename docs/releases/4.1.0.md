...
- **Status:** published
- **npm:** `arkgate@4.1.0` (superseded by `4.1.1` on `latest`)
- **Git tag:** `v4.1.0`
...
Re-verified against mother CLI **4.1.0** (now on npm `latest`) on a hostile brownfield cohort:
...
3. [x] Signed tag `v4.1.0` + GitHub Release from this file  
4. [x] `gh workflow run publish-npm.yml -f tag=v4.1.0 -f dry_run=false`  
5. [x] Verify: `npm view arkgate@4.1.0 version` and `npm view arkgate dist-tags.latest` → `4.1.0`  
