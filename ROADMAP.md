...
### Phase LD — Layer description projection (next train)
...
`K01`. No new skill names. Absence never invents residual. Engineering doing: none — `LD01`–`LD04` done (doctor / coverage / HTML project layer descriptions). Next is `LD05`.

The caption is already on the contract. Place / prepare-write / MCP JSON, doctor, coverage, and the HTML Purpose column include it when present.
...
Engineering doing: none. Phase **LD** `LD01` **done** (ADR 0035); `LD02` **done** (policyHash omits layer descriptions); `LD03` **done** (place JSON projects `description`); `LD04` **done** (doctor / coverage / HTML caption parity); `LD05`–`LD06` **todo**. Phase **LV** `LV01`–`LV09` **done** (4.8.6 published). `AGY01` **done** (4.8.5 published). `OR01`–`OR07` **done**. `WH01` **done** (ADR 0026). `PK01` **done**.
