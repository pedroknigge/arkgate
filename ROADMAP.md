...
- New skill *names* beyond the closed Domain catalog (`ARK_SKILL_NAMES`). Prefer deepen +
  route. **Exception:** Issue [#216](https://github.com/pedroknigge/arkgate/issues/216) /
  `SK216` / [ADR 0036](docs/adr/0036-skill-catalog-product-capacity.md) opens ACS05 so the
  set covers 100% of product capacity (first-class `/ark-order`; one-release stubs).
  Further names still need a live ROADMAP item. **IC05** deepened bodies — same era names.
...
Engineering doing: none — 4.8.8 publication complete. `RL88`, `ST02`, `ST01`, and `AO09` done; Phase LV done.
...
| 224 | `LV02` | `done` | L | LV01 | `apply(ProposeResult)`; unvalved second freeze fails `ARKORDER_UNVALVED_RELEASE` |
| 225 | `LV03` | `done` | L | LV01 | `xiHash` vs σ; `refreshSigma`; stale σ → residual `hold` (not a new Release) |
| 226 | `LV04` | `done` | L | LV02+LV03 | Ingest residual bound to `xiHash`; closed `absorb \| escalate_up \| hold` + `reasonCode` |
| 227 | `LV05` | `done` | M | LV04 | Capacity pack as data (`kind` / `sigmaKey` / `payloadKey` / `op`); no user predicates |
...
| Order | ID | Status | Size | Depends on | Outcome |
|---:|---|---|---:|---|---|
| 241 | `RL88` | `done` | M | ST02 | Reconciled 4.8.7 with the dashboard train and honesty copy; verified, signed, published npm + MCP Registry, updated the website, and verified every public pointer. Does not close `Z09` / `K01` |

### Patch 4.8.9 — doctor default agent home

`--doctor` on a consumer tree throws `TypeError: The "paths[0]" argument must be of type string. Received undefined` when `CLAUDE_HOME` / `GROK_HOME` / `ANTIGRAVITY_HOME` are unset. `skillsDirFor` calls `resolveHomeDir` without `homeDir` since the 4.8.5 Antigravity refactor. The mother tree never hits the path (`templates/skills` short-circuit); existing tests always inject the env vars.
...
Engineering: `RL810` **done**. `HINT01` `HINT02` `YAML01` `SH01` `DS01` `WA01` `SC01`
`BK01` `IR01` `MSG01` `IN01` published in **4.8.10**.
...
| Order | ID | Status | Size | Depends on | Outcome |
|---:|---|---|---:|---|---|
| 267 | `RL810` | `done` | M | IN01 | Prepare, tag, and publish **4.8.10** (HINT01–IN01 + report CLI). Does not close `Z09` / `K01` |

### Patch 4.8.11 — nested `--config` + Shape dogfood

Consumer CLI: nested `--config` with `--root` stays inside that tree. Library
Shape: `adapterContract` + `agentProjection` god-module splits. CI:
`doctorResidentWarm` ceiling 680ms. Dev-tree pins: `browserslist` 4.28.8,
`fast-uri` 3.1.7. No `schemaVersion` bump. No new skill names. Does not close
`Z09` / `K01`. Branch `release/4.8.11` merged.

Engineering: `RL811` **done**.

### Release 4.8.11 — complete publish
...
No `schemaVersion` bump. Does not close `Z09` / `K01`. Mother `ark.config.json`
stays extras-off. Merged as #206; publish cut is **4.8.12**.

| Order | ID | Status | Size | Depends on | Outcome |
|---:|---|---|---:|---|---|
| 269 | `AO17` | `done` | S | RL811 | Doctor/status/HTML `arkOrder` (`notAScore`); `[ArkOrder]` on check denies; start extras-off line; merge-planes honesty names ArkOrder; billing gallery one-breath + skip-corpus proof |

### Release 4.8.12 — prepare publish

| Order | ID | Status | Size | Depends on | Outcome |
|---:|---|---|---:|---|---|
| 270 | `RL812` | `done` | S | AO17 | Prepare **4.8.12** (ArkOrder first-contact honesty from #206). Tag `v4.8.12` cut at `28ac035`. `publish-npm` failed branch coverage (80.97% < 81%). Do not rewrite the tag. Do not publish 4.8.12 to npm — ship is **4.8.13**. Does not close `Z09` / `K01` |
...
| Order | ID | Status | Size | Depends on | Outcome |
|---:|---|---|---:|---|---|
| 271 | `RL813` | `done` | S | RL812 | Prepare **4.8.13** (remaining ArkOrder doctor/deny branches so the 81% floor holds). Tag `v4.8.13` and GitHub Release exist. Published on npm `latest`. Skip 4.8.12 on npm. Does not close `Z09` / `K01` |

### Release 4.8.14 — catch npm to main

`v4.8.13` is on npm `latest`. This thin patch ships first-class `/ark-order`
and the other `main` ships after that tag. No `docs/releases/4.8.14.md`.
...
| Order | ID | Status | Size | Depends on | Outcome |
|---:|---|---|---:|---|---|
| 291 | `RL817` | `done` | S | AP02 | Prepare **4.8.17** (daily accumulate #264, #265). Tag `v4.8.17` and GitHub Release exist. Published on npm `latest`. Does not close `Z09` / `K01` |

### Release 4.8.18 — daily accumulate

Thin patch: six ships after 4.8.17 published — monorepo DomainModel
globs (#269), packed README latest-truth (#270 / #272), deep pstack
audit (#273), doctor #1 dual-match honesty (#275), compact-start size
refuse (#274), and stale-skill Guiar (#278). No `docs/releases/4.8.18.md`.
Does not close `Z09` / `K01`.

| Order | ID | Status | Size | Depends on | Outcome |
|---:|---|---|---:|---|---|
| 292 | `RL818` | `done` | S | RL817 | Prepare **4.8.18** (daily accumulate #269, #270/#272, #273, #275, #274, #278). Tag `v4.8.18` and GitHub Release exist. Published on npm `latest`. Does not close `Z09` / `K01` |

### Release 4.8.19 — daily accumulate

Thin patch: five ships after 4.8.18 published — pre-hook write gate
first (#280), honest workspace evidence + playground exclude (#286),
skill stamp refresh (#285), honest start docs and install-fail next
step (#287), and honest recommend label when uiHeavy (#289). No
`docs/releases/4.8.19.md`. Does not close `Z09` / `K01`.

| Order | ID | Status | Size | Depends on | Outcome |
|---:|---|---|---:|---|---|
| 294 | `RL819` | `done` | S | RL818 | Prepare **4.8.19** (daily accumulate #280, #286, #285, #287, #289). Tag `v4.8.19` and GitHub Release exist. Published on npm `latest`. Does not close `Z09` / `K01` |

### Release 4.8.20 — daily accumulate

Thin patch: ships after 4.8.19 published — `coverage.symbol` is a non-test
declaration (#301 / #291), reshape pilot stays in the source layer and
slice (#299 / #295), physical-cohesion residual clears finished and
elegant (#302 / #296), anchored starred `sliceFolders` (#300 / #294),
shared root cannot import a slice when opted in (#303 / #297), pnpm Age
install-fail recovery (#293 / #292), vitest 4.1.11 (#298), Dependabot
vitest 5 ignore (#304), and docs (#244, #234). No
`docs/releases/4.8.20.md`. Does not close `Z09` / `K01`.

| Order | ID | Status | Size | Depends on | Outcome |
|---:|---|---|---:|---|---|
| 295 | `RL820` | `done` | S | RL819 | Prepare **4.8.20** (daily accumulate #301/#291, #299/#295, #302/#296, #300/#294, #303/#297, #293/#292, #298, #304, #244, #234). Tag `v4.8.20` and GitHub Release exist. Published on npm `latest`. Does not close `Z09` / `K01` |

### Release 4.8.21 — daily accumulate

Thin patch: ships after 4.8.20 published — a bare invariant-id mention is
not coverage and `coverage.symbol` accepts `type` / `interface` / `enum`
(#319 / #310, #307), explained `upgrade --apply` outcome with exit 3 for
applied but red (#320 / #311, #312), optional `sliceIdentity: "stars"`
(#318 / #308), grouped `SHARED_IMPORTS_SLICE` output (#316 / #313), pilot
loop sees reshape pilots (#317 / #309), README first contact (#306), and
docs fixes (#315). No `docs/releases/4.8.21.md`. Does not close `Z09` /
`K01`.

| Order | ID | Status | Size | Depends on | Outcome |
|---:|---|---|---:|---|---|
| 296 | `RL821` | `doing` | S | RL820 | Prepare **4.8.21** (daily accumulate #319/#310/#307, #320/#311/#312, #318/#308, #316/#313, #317/#309, #306, #315). After merge: annotated tag `v4.8.21` + GitHub Release (no npm claim until `npm view arkgate@4.8.21 version` confirms, #314) + `gh workflow run publish-npm.yml -f tag=v4.8.21 -f dry_run=false`. Does not close `Z09` / `K01` |
...
| Order | ID | Status | Size | Depends on | Outcome |
|---:|---|---|---:|---|---|
| 293 | `PH01` | `done` | S | WG01 | Host pre-hook first; MCP prepare fallback; compact Cursor requires the fail-closed hook; doctor/start name the fallback; host matrix documents hard vs advisory vs MCP-only. #277 / #280 |
...
| Order | ID | Status | Size | Depends on | Outcome |
|---:|---|---|---:|---|---|
| 274 | `WG01` | `done` | S | — | Cursor hard-write inventory requires `failClosed: true`; doctor/deny honesty names fail-open; `--require-write-hook cursor` fails on a fail-open hook. Host-neutral policy on the existing matrix. #231 CI green. |

### Patch — unclassified include is not a green check

Parked 4.8.4 residual: an `include` that classifies no file still passed green.
`ANALYSIS_COVERS_NO_FILES` already named include **and** layer; the include-only
walk left a vacuous pass. Dual-gate: write hook allowed those files; CI
`--strict-merge` did not. No `schemaVersion` bump. No new skill. Does not close
`Z09` / `K01`.

| Order | ID | Status | Size | Depends on | Outcome |
|---:|---|---|---:|---|---|
| 275 | `UC01` | `done` | S | WG01 | Refuse when include matches files and zero are classified; write hook denies the same files. Partial unclassified stays a warning. Report modes still diagnose. #233 |

### Patch — write hook layer specificity matches ark-check (issue #237)

Hook and CI must name the same layer when an explicit file pattern and a
broader glob both match. Agents were refactoring away from a design
`ark-check` already allowed. No `schemaVersion` bump. No new skill. Does
not close `Z09` / `K01`.
...
and Contener · Guiar · Ordenar. Skill count is secondary. ACS05 freeze opened for
this item only ([ADR 0036](docs/adr/0036-skill-catalog-product-capacity.md)).
Does not close `Z09` / `K01`. No `schemaVersion` bump. Extras stay off by default.
Skills never enforce.

| Order | ID | Status | Size | Depends on | Outcome |
|---:|---|---|---:|---|---|
| 272 | `SK216` | `done` | M | RL811 | First-class `/ark-order`; retire shortcuts as one-release stubs; north-star routing; standing capacity check; `check:agent-skills` fails closed when a product plane is missing from skill bodies |
...
Engineering doing: `RL821`. `AP02` **done**. Patch 4.8.21 **prepared**. Patch 4.8.20 **published**. Patch 4.8.19 **published**. Patch 4.8.18 **published**. Patch 4.8.17 **published**. `DF243`/`DF246`/`DF247` **done** (4.8.15 slice 2 — dogfood honesty). `LC01` **done**. `SC01` **done**. `RL816` **done** (4.8.16 on npm `latest`). `RL815` **done** (4.8.15 published). `RL814` **done** (4.8.14 published). `RL813` **done** (4.8.13 published). `RL812` **done** (tag `v4.8.12` cut; publish failed coverage; skip on npm). Patch 4.8.12 **tagged, not published**. `AO17` **done**. `RL811` **done** (4.8.11 published). `RL810` **done** (4.8.10 published). `RL89` **done** (4.8.9 published). `AH01`/`CS01`/`CI01`/`UP01`/`PX01`/`INT01` **done**. Amarilla ArkOrder `AO10`–`AO16` **done**. `RL88` **done** (4.8.8 published). Phase **LD** `LD01`–`LD06` **done** (4.8.7 published). `ST02`, `ST01`, and `AO09` **done**. Phase **LV** `LV01`–`LV09` **done** (4.8.6 published). `AGY01` **done** (4.8.5 published). `OR01`–`OR07` **done**. `WH01` **done** (ADR 0026). `PK01` **done**.
`K01` **parked** (4.7.6 primitives only).
Phase DX (`DX01`–`DX03`) **done** and present in published `arkgate@4.8.0+` (4.7.6 predates them).
Phase HS (`HS01`–`HS05`) **done**. Remaining first-contact copy **4.7.3** published. Write. Check. Ship. patch **4.7.5** published. **4.8.11** is on npm `latest`. 4.8.13 is published. 4.8.14 is published. 4.8.15 is published. 4.8.16 is published. 4.8.17 is published. 4.8.18 is published. 4.8.19 is published. 4.8.20 is published. 4.8.21 is prepared. Tag `v4.8.12` stands; skip that version on npm.
...
### Phase XP — ArkOrder x ArkRun convergence (shipped in 4.8.5)

Plan: [docs/plans/arkorder-arkrun/README.md](docs/plans/arkorder-arkrun/README.md).

Opened because two adopters, in separate sessions with no contact, both concluded "ArkOrder, no
documentation" and then requested features that already ship (`IngestEscalate`, `Projector`,
`maxXiKeys` + its sensors, `ProposeResult`, `XiSchema`). A surface nobody can find does not exist.
`XP01` outranks every feature here.

Measured, and it makes the convergence cheap: both extras already share `mode` and `managedLayers`
and diverge only on roots naming (`planeRoots` vs `compositionRoots`/`kernelRoots`). The activation
model is already one shape; it is not named, documented or tested as one.
