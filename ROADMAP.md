...
- **Status date:** 2026-08-25 (Engineering doing: `RN04`; `RN03` **done**; `RN02` **done**; `RN01` **done`; Phase AL `AL01`–`AL04` **done`; `AL06` compact-doctor extract **done`; `AL05` parked; Phase RN ArkRun started; Z09 still parked; tree **arkgate@4.6.7**; npm `latest` is **4.6.7**)
...
Engineering doing: `RN04` (Tier-1 ArkRun sensors; advisory does not flip `valid`; enforced blocks). `RN03` **done** (resolver facts for kernel call sites, managed `new`, composition-root hits; facts schema additive).
...
| 188 | `RN04` | `doing` | L | RN03 | Tier-1 ArkRun sensors; advisory does not flip `valid`; enforced blocks |
