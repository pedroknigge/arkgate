...
| 38 | `T04` | `doing` | M | `T03` | Read-only convergence reports planned, missing, contradictory, and unplanned structural impact |
...
**Next:** Phase T is active with `T04` as the only `doing` item. `T03` is green on pushed commit
`0aba7cc`; start `T05` only after `T04` is reviewed, verified, committed, and green in CI. Scope and acceptance:
...
```text
Item: T04 doing — honest read-only structural convergence
First result: failing fixtures for satisfied, missing, contradictory, and unplanned structural impact
Then: compare the explicit T03 map with actual candidate paths/edges without writing or claiming behavior
Primary surfaces: pure DomainModel convergence IR, shared preflight adapter, CLI/MCP parity
Required finish: four outcome classes + clean result + byte-for-byte no-write proof + exact-SHA CI
