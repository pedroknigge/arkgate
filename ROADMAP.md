...
`K01`. No new skill names. Absence never invents residual. Engineering doing: none —
first `doing` is `LD01` when picked up.

The caption is already on the contract. Place / doctor / coverage do not show it.
`policyHash` treats a copy tweak as a contract change. Grain is the layer, not each file.

| Order | ID | Status | Size | Depends on | Outcome |
|---:|---|---|---:|---|---|
| 232 | `LD01` | `todo` | M | — | ADR 0035: reuse `layers[].description`; strip from policyHash; project; absence silent |
| 233 | `LD02` | `todo` | M | LD01 | `policyHash` omits layer descriptions; caption-only delta is neutral |
...
Engineering doing: none. Phase **LD** (`LD01`–`LD06`) **todo**. Phase **LV** `LV01`–`LV09` **done** (4.8.6 published). `AGY01` **done** (4.8.5 published). `OR01`–`OR07` **done**. `WH01` **done** (ADR 0026). `PK01` **done**.
