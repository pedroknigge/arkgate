...
- **Status date:** 2026-08-26 (Engineering doing: none; Phase HS `HS01`–`HS05` **done**; `RN16` **done**; `RN17` **blocked** on npm org `@arkgate` + `NPM_TOKEN`; Phase RN ArkRun shipped **4.7.0**; Phase HS shipped **4.7.1**; wording patch **4.7.2** published; remaining first-contact copy **4.7.3** published; `arkgate@4.7.4` on npm `latest`; `AL05` parked; Z09 still parked; npm `latest` is **4.7.4**)
...
`RN16` **done** (public docs + 4.7.0 published). `RN17` **blocked** (companion pipeline landed; first `@arkgate/runtime` npm copy needs org + `NPM_TOKEN`).
...
Phase RN shipped in **4.7.0** (gate extra). Companion registry publish is `RN17` (blocked on npm org + token). Phase HS is the **4.7.1** host-catalog patch.
...
| 206 | `RN17` | `blocked` | M | RN16 | Pipeline + `arkgate@4.7.4` on `latest`. First `@arkgate/runtime` registry copy needs npm org `@arkgate` + GitHub `NPM_TOKEN` (OIDC trusted publishing is only registered for unscoped `arkgate`). Then `publish-runtime.yml`. Does not close Z09/K01 |
