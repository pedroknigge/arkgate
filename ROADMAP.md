...
fail-closed. Later slices: dogfood #243 / #246 / #247. Does not close
`Z09` / `K01`. No `schemaVersion` bump. No new skill.

| Order | ID | Status | Size | Depends on | Outcome |
|---:|---|---|---:|---|---|
| 282 | `LC01` | `doing` | S | SC01 | Opt-in `--local` / `ARK_CHECK_LOCAL=1` reuses `--changed`; refuse `--strict-merge`; per-root isolation proof; write hooks stay lexical |
| 283 | `DF243` | `todo` | S | LC01 | Dogfood #243: type-only `LAYER_IMPORT_VIOLATION` glyph vs exit 0 (later slice) |
| 284 | `DF246` | `todo` | S | LC01 | Dogfood #246: `start --apply` refuse UX on the one-minute path (later slice) |
| 285 | `DF247` | `todo` | S | LC01 | Dogfood #247: start vs `--recommend` projected coverage (later slice) |
