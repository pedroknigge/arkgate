 ...
 * Wires the Structrail kernel, the adapters, and the use case together.
 */
import { createStructrailKernel } from 'structrail/runtime';
...
const structrail = createStructrailKernel();

// 1. Intents
const OrderPlaced = structrail.registry.define<typeof ORDER_PLACED, OrderPlacedPayload>(ORDER_PLACED);
structrail.registry.define<typeof PLACE_ORDER, { orderId: string }>(PLACE_ORDER, {
...
structrail.projections.register({
  name: 'Orders',
...
const placeOrder = createPlaceOrder({
  repository,
  publisher: structrail.publisher(PLACE_ORDER),
...
const server = createHttpServer({
  placeOrder,
  getOrder: async (id) => {
    const state = (await structrail.projections.getState('Orders')) as { orders: Record<string, OrderView> };
...
server.listen(port, () => {
  ...
  console.log(`kernel=${structrail.instanceId} profile="${structrail.profile.name}"`);
});
