      ...
      SessionStart: [
        {
          hooks: [
            {
              type: 'command',
              command: `${runner} ${identity.mcpBin} --session-context --root "$CLAUDE_PROJECT_DIR" --config ${identity.configName}`,
            },
          ],
        },
      ],
          ...
          hooks: [
            {
              ...
              command: `${runner} ${identity.mcpBin} --hook --hook-repair --root "$CLAUDE_PROJECT_DIR" --config ${identity.configName}`,
            },
          ],
        },
      ],
...
export function grokProjectConfig(root, identity = ARK_GENERATION_IDENTITY) {
  ...
  return `# Generated by ${identity.checkBin} --install-agent-gates (Grok Build project scope).
...
export function grokHooks(root, identity = ARK_GENERATION_IDENTITY) {
  const runner = execRunner(root);
          ...
          hooks: [
            {
              type: 'command',
              timeout: 30,
              command: `${runner} ${identity.mcpBin} --session-context --root "${grokRoot}" --config ${identity.configName}`,
            },
          ],
        },
      ],
          ...
          hooks: [
            {
              ...
              command: `${runner} ${identity.mcpBin} --hook --hook-repair --root "${grokRoot}" --config ${identity.configName}`,
            },
          ],
        },
      ],
