...
| [layer-description-projection](docs/plans/layer-description-projection/README.md) | Shipped on tree in **4.8.7** (`LD01`–`LD06` done; not npm-published from this item) | Project existing `layers[].description` to place/doctor/report; strip from policyHash; app-context caption; absence silent. No new key, no new skill |
