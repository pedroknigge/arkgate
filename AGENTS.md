...
| [layer-description-projection](docs/plans/layer-description-projection/README.md) | Prepared (Phase LD; `LD01`–`LD06` `todo`; target **4.8.7**) | Project existing `layers[].description` to place/doctor/report; strip from policyHash; app-context caption; absence silent. No new key, no new skill |
