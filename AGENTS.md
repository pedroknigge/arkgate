...
| [arkorder-valve-loop](docs/plans/arkorder-valve-loop/README.md) | Shipped on tree in **4.9.0** (`LV01`–`LV09` done; not npm-published from this item) | Haken control loop: valve (`apply`), σ identity, ingest residual, capacity-as-data, ArkRun decision tape, `ReleaseStore` port. [ADR 0034](docs/adr/0034-arkorder-valved-loop.md). Does not close Z09/K01. No Orderfield CLI |
