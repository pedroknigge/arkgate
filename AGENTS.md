...
| [layer-description-projection](docs/plans/layer-description-projection/README.md) | In progress (Phase LD; `LD01` done — [ADR 0035](docs/adr/0035-layer-description-projection.md); `LD02`–`LD06` `todo`; target **4.8.7**) | Project existing `layers[].description` to place/doctor/report; strip from policyHash; app-context caption; absence silent. No new key, no new skill |
